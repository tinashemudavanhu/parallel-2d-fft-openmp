...
#ifndef FFT2D_OMP_THRESHOLD
#define FFT2D_OMP_THRESHOLD 65536
#endif
    ...
    int *ip, double *w)
{
    ...
    if ((double) n1 * n2 >= (double) FFT2D_OMP_THRESHOLD) {
    ...
    int *ip, double *w)
{
    ...
    if (n2 > 4) {
        #pragma omp parallel if ((double)n1 * n2 >= (double)FFT2D_OMP_THRESHOLD)
        {
            double *tmp = (double *) malloc(sizeof(double) * 8 * n1);
            fft2d_alloc_error_check(tmp);
            ...
            for (j = 0; j < n2; j += 8) {
                for (i = 0; i < n1; i++) {
                    tmp[2 * i] = a[i][j];
                    tmp[2 * i + 1] = a[i][j + 1];
                    tmp[2 * n1 + 2 * i] = a[i][j + 2];
                    tmp[2 * n1 + 2 * i + 1] = a[i][j + 3];
                    tmp[4 * n1 + 2 * i] = a[i][j + 4];
                    tmp[4 * n1 + 2 * i + 1] = a[i][j + 5];
                    tmp[6 * n1 + 2 * i] = a[i][j + 6];
                    tmp[6 * n1 + 2 * i + 1] = a[i][j + 7];
                }

                cdft(2 * n1, isgn, tmp, ip, w);
                cdft(2 * n1, isgn, &tmp[2 * n1], ip, w);
                cdft(2 * n1, isgn, &tmp[4 * n1], ip, w);
                cdft(2 * n1, isgn, &tmp[6 * n1], ip, w);

                for (i = 0; i < n1; i++) {
                    a[i][j] = tmp[2 * i];
                    a[i][j + 1] = tmp[2 * i + 1];
                    a[i][j + 2] = tmp[2 * n1 + 2 * i];
                    a[i][j + 3] = tmp[2 * n1 + 2 * i + 1];
                    a[i][j + 4] = tmp[4 * n1 + 2 * i];
                    a[i][j + 5] = tmp[4 * n1 + 2 * i + 1];
                    a[i][j + 6] = tmp[6 * n1 + 2 * i];
                    a[i][j + 7] = tmp[6 * n1 + 2 * i + 1];
                }
            }
    ...
    double *t, int *ip, double *w)
{
    ...
    if (n2 > 2) {
        #pragma omp parallel if ((double)n1 * n2 >= (double)FFT2D_OMP_THRESHOLD)
        {
            double *tmp = (double *) malloc(sizeof(double) * 4 * n1);
            fft2d_alloc_error_check(tmp);
            ...
            for (j = 0; j < n2; j += 4) {
                ...
                if (ics == 0) {
                    ddct(n1, isgn, tmp, ip, w);
                    ddct(n1, isgn, &tmp[n1], ip, w);
                    ddct(n1, isgn, &tmp[2 * n1], ip, w);
                    ddct(n1, isgn, &tmp[3 * n1], ip, w);
                } else {
                    ddst(n1, isgn, tmp, ip, w);
                    ddst(n1, isgn, &tmp[n1], ip, w);
                    ddst(n1, isgn, &tmp[2 * n1], ip, w);
                    ddst(n1, isgn, &tmp[3 * n1], ip, w);
                }
